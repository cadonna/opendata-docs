...
```bash
tar -xzf node_exporter-*.tar.gz && rm node_exporter-*.tar.gz
cd node_exporter-*

# on macOS you might need to execute `xattr -d com.apple.quarantine ./node_exporter` 
# before the next line to avoid macOS blocking the binary
./node_exporter
```
